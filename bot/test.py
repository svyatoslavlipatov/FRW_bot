...
def create_account_table(user_id, telegram_username):
    ...
    if not existing_user:  # Если пользователь еще не существует, то добавляем
        cursor.execute(
            '''INSERT INTO account (user_id, telegram_username, start_time, last_login_time) VALUES (?, ?, ?, ?)''',
        ...
        cursor.execute('''UPDATE account SET last_login_time = ? WHERE user_id = ?''', (current_time, user_id))
    connection.commit()
    connection.close()
